...
function ctck()
{
     ...
     if(sds == null){
        uifatrt("You are using a free package.\n You are not allowed to remove the tag.\n");
     }
...
echo "<th>". $log['type']. "</th>";
echo "<th><a href=\"https://rynantech.com/partner-login/uifatclick.php?id=".$log['id']."\"><img src='https://rynantech.com/partner-login/sites/images/dl.png".$row['image_name']."'></a></th>";
...
//echo "<td><a href=\"https://rynantech.com/partner-login/uifat/VJET/$log[link]\" onclick=\"khoa();\">$log[link]</a></td>";
...
//echo  @mysql_num_rows( $sql_query ); die();
$logs = @mysql_query("SELECT * from uifat where printers like 'RYNAN R20 MAX' and type like 'software' order by timecreate desc");
while($log = mysql_fetch_array( $logs )){
// Bo gio - chi lay ngay
$date = $log['timecreate'];
$createDate = date('Y-m-d', strtotime($date));
...
//echo  @mysql_num_rows( $sql_query ); die();
$logs = @mysql_query("SELECT * from uifat where printers like 'W1000' and type like 'brochure' order by timecreate desc");
while($log = mysql_fetch_array( $logs )){
// Bo gio - chi lay ngay
$date = $log['timecreate'];
$createDate = date('Y-m-d', strtotime($date));
...
<div class="tab">
  ...
  <button class="tablinks" onclick="openCity(event, 'RYNAN R20 MAX-pr')">RYNAN R20 MAX</button>
</div>
...
<div id="RYNAN R20 MAX-pr" class="tabcontent" style="display: block;" >
<table id="tracelog1" width="909" border="0" align="center">
<?php
