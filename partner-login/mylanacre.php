...
if ( $_GET['act'] == "do" )
{
   ...
   // $vercode = addslashes( $_POST['vercode'] );
    ...
    if ( mysql_num_rows(mysql_query("SELECT filename FROM mylana WHERE filename='$filename'"))>0)
    {
    ...
    @$a=mysql_query("INSERT INTO mylana (filename, type, printers, link, notes, timecreate) VALUES ('{$filename}', '{$type}', '{$printers}', '{$link}', '{$notes}', '{$timereg}')");
...
<table border="0" cellpadding="0" cellspacing="0" width="977">
	...
	<div><p class="backtoindex"><a href="/partner-login/mylanacp.php" class="hrlink"><font size="12" color="#fff" class="rightpnhome">Back to index</p></font></a></div>


	<tr>
		<td>
		<p align="justify" style="margin: 12px 22px; margin-left: 322px;"><b><span style="font-size: 16px;">Please enter the information link mylana</span></font></b></p>
...
<?php
 // Thông báo hoàn tất việc tạo tài khoản
    if ($a)
        print "The files {$username} has been created <a href='mylanacp.php'>.     Click here to list mylana</a>";
    else
        print " ";
...
<form action="mylanacre.php?act=do" method="post">
